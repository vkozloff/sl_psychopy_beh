...
all_lsl<-subset(indiv_accuracies, task=="LSL")
...
all_SSL<-subset(indiv_accuracies, task=="SSL")
task <- append (task, paste ("SSL"))
mean_accuracy <- append (mean_accuracy, round(mean(all_SSL$accuracy), digits = 3))

# Find mean TSL accuracy across participants
all_TSL<-subset(indiv_accuracies, task=="TSL")
task <- append (task, paste ("TSL"))
mean_accuracy <- append (mean_accuracy, round(mean(all_TSL$accuracy), digits = 3))

# Find mean VSL accuracy across participants
all_VSL<-subset(indiv_accuracies, task=="VSL")
...
setwd("../output_files/")
write.csv(group_accuracy, "nov_pilot_accuracy_group.csv")
